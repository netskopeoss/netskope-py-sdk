...
from netskope.resources.events.decoder import _parse_page
...
def _update_result(body: Any) -> IncidentUpdateResult:
    ...
    if isinstance(raw, list):
        ...
        # incident_update_404_response (incident_update.yaml:76-82) reports a
        # bare {ok, result} outcome rather than the 200 result envelope.
        entries = [body]
    else:
        raise ValueError("Expected incident update outcomes.")
    if not entries:
    ...
    for entry in entries:
        ...
        # incident_update.yaml:8-14 requires neither property and bounds
        # neither, so a sparse entry or an unfamiliar flag value decodes and is
        # read as "not accepted" rather than rejecting the acknowledgement.
