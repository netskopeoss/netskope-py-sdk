...
from netskope.resources.nsiq.decoder import AsyncNsiqResponses, NsiqResponses
...
# ``GET /url/recategorizations`` (nsiq/url_recategorization.yaml:289-369)
# declares ``limit`` as ``minimum: 1, maximum: 10``, ``offset`` as
# ``minimum: 0``, and enumerates ``status``, ``sortby`` and ``sortorder``.
_RECAT_LIMIT_RANGE = (1, 10)
_RECAT_STATUSES = ("received", "inprogress", "completed", "rejected")
_RECAT_SORT_FIELDS = ("task_id", "start_time", "end_time", "status")
_RECAT_SORT_ORDERS = ("asc", "desc")
...
def _build_recat_list_params(
    ...
    params: dict[str, Any] = {
        "offset": offset,
        "limit": limit,
        "sortby": _one_of("sort_by", sort_by, _RECAT_SORT_FIELDS),
        "sortorder": _one_of("sort_order", sort_order, _RECAT_SORT_ORDERS),
    }
    ...
    if status is not None:
        params["status"] = _one_of("status", status, _RECAT_STATUSES)
    return params
