...
import netskope.resources.dem.paths as api
...
def _paging(
    ...
    offset: int | None,
    *,
    limit_range: tuple[int, int | None] = (0, None),
) -> dict[str, Any]:
    """Build a ``limit``/``offset`` query bounded as the operation declares.

    ``limit_range`` defaults to "zero or more" for the operations that state no
    bound; pass the declared pair where one exists.
    """
    params: dict[str, Any] = {}
    if limit is not None:
        params["limit"] = api._bounded("limit", limit, *limit_range)
    if offset is not None:
        params["offset"] = api._bounded("offset", offset, 0, None)
    return params
...
def _dataset_body(
    ...
    if end_ms - start_ms > 48 * 3600 * 1000:
        ...
        # (dem-workbench-query.yaml:918, inside the /query/getdataset description
        # that starts at :893).
        raise ValidationError("The dataset time range must not exceed 48 hours.")
    # ``DataSetQueryInput`` bounds limit and offset (dem-workbench-query.yaml:73-87);
    # ``api._getdata_body`` applies the same pair for ``/query/getdata``.
...
class DemProbeResponses(_DemResponses):
    def list(
        ...
        return self._response(
            ...
            params=_paging(limit, offset, limit_range=api._PROBE_LIMIT_RANGE),
        )
...
class DemAlertResponses(_DemResponses):
    ...
    def entities(
        ...
        *,
        ...
        sort_by: str | None = None,
        sort_order: str | None = None,
    ) -> ApiResponse[builtins.list[DemAlertEntity]]:
        params = _paging(limit, offset)
        if sort_by is not None:
            params["sortby"] = sort_by
        if sort_order is not None:
            params["sortorder"] = api._enum_value("sort_order", sort_order, SORT_ORDERS)
        return self._response(
            "GET",
            f"{api._ALERTS_PATH}/{quote_id(alert_id)}/entities",
            _collection(DemAlertEntity, "entities"),
            params=params,
        )
...
class DemAppResponses(_DemResponses):
    def list(
        ...
        *,
        ...
        params = _paging(limit, offset, limit_range=api._APP_LIMIT_RANGE)
        if name is not None:
            params["name"] = name
