...
:class:`netskope.resources.scim.resource.ScimResource`.
...
from netskope.resources.users.decoder import (
...
def _build_query_body(
    ...
    The paging block is built through :class:`~netskope.models.users.UserQuery`,
    so these methods enforce the same bounds the typed ``list_page`` twins do:
    ``Pagination.limit`` is ``minimum: 0, maximum: 1000`` and
    ``Pagination.offset`` is ``minimum: 0``
    (users/usermanager.yaml:1190-1204), and a value outside them is refused
    before any request.
    """
    try:
        query = UserQuery(filter=filter, limit=limit, offset=offset)
...
class UserGroupsResource(SyncResource):
    ...
    :class:`netskope.resources.scim.resource.ScimGroupsResource`.
    """
    ...
    def get(self, name: str) -> UmGroup | None:
        ...
        The ``displayName`` filter is inferred from the ``Group.displayName``
        property (usermanager.yaml:908-918) and from ``Filter``'s
        ``additionalProperties`` (:1243-1280), which makes any field name
        schema-valid. It is not one of the four documented getgroups filters
        (:106-146), which use ``scimId``, ``id``, ``deleted`` and
        ``collectionId``; whether the service indexes groups by display name is
        not something the contract states. Prefer ``list(filter=...)`` with
        ``id``; the spec does show that one filtered, with ``co`` (:116-126) :
...
class UsersResource(SyncResource):
    ...
    :class:`netskope.resources.scim.resource.ScimUsersResource`.
    """
