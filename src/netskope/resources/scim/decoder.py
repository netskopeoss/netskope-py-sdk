...
from netskope.resources.shared.admin import page, page_params
...
def user_patch_operations(fields: dict[str, Any]) -> list[dict[str, Any]]:
    """One path-scoped replace per attribute, as nearly every spec example is.

     ``PATCH /Users/{id}`` documents its operations per attribute path
     (scim-apis.yaml:1895-1925) and its ``path`` enum names each one
     (:1950-1973). Of the 29 request examples at :1994-2264, every one but
     ``replaceManager`` (:2201-2211) carries a path; that one nests the
     attribute inside ``value`` instead, a shape this SDK does not emit. The
     same change is reachable through the enum's
     ``…extension:enterprise:2.0:User:manager`` path (:1969), so nothing is lost
    ; but the spec never says whether ``path`` is required, so a tenant that
     insists on the nested form needs a hand-built body.
    """
