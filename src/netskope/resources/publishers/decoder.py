...
class PublisherResponses(SyncResource):
    ...
    def update_alerts_configuration_request(
    ...
    ) -> ApiResponse[PublisherAlertsConfigurationStatus]:
        """Replace the alert configuration; parses the status acknowledgment.

        ``publishers_alert_put_response`` (``npa_publishers.yaml:630-638``)
        declares only ``status``, so the result is an acknowledgment.  Use
        :meth:`get_alerts_configuration` to read the stored configuration back.
        """
        ...
        return ApiResponse(
            response, lambda raw: parse_item(raw.json(), PublisherAlertsConfigurationStatus)
        )
    ...
    def list_page(
        ...
        *,
        ...
        ``getNPAPublishers`` declares only ``fields``
        (``npa_publishers.yaml:1024-1032``), so the request sends only that and
        the window is applied to the whole collection the gateway answers with.
        """
    ...
    def bulk_upgrade(self, publisher_ids: list[int | str]) -> ApiResponse[PublisherActionResult]:
        """Request an upgrade and parse its result, including the publisher records.

        ``publishers_bulk_response`` (``npa_publishers.yaml:639-702``) carries
        the updated publishers under ``data.publishers`` beside its ``status``;
        both reach :class:`PublisherActionResult`.
        """
...
class AsyncPublisherResponses(AsyncResource):
    ...
    async def update_alerts_configuration_request(
        ...
        See :meth:`PublisherResponses.update_alerts_configuration_request`.
        """
        payload = request_payload(request, PublisherAlertsConfigurationPatch)
    ...
    async def list_page(
        ...
        *,
        ...
        """Fetch one locally-applied window.  See :meth:`PublisherResponses.list_page`."""
        _validate_window(offset, limit)
        params = _build_list_params(filter_expr, fields)
