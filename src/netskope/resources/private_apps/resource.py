...
from netskope.resources.private_apps.decoder import (
...
class PrivateAppTagsResource(SyncResource):
    ...
    def get_policy_in_use(self, tag_ids: builtins.list[int]) -> dict[str, Any]:
        ...
        ``POST /apps/private/tags/getpolicyinuse`` declares
        ``rbac.access: rw`` (``npa_private_tag.yaml:487-492``), so despite the
        read-sounding name it is not replayed on a 429 or 5xx.
        """
        return self._post(_TAGS_POLICY_IN_USE_PATH, json={"ids": id_strings(tag_ids, "tag_ids")})
...
class AsyncPrivateAppTagsResource(AsyncResource):
    ...
    async def get_policy_in_use(self, tag_ids: builtins.list[int]) -> dict[str, Any]:
        ...
        See :meth:`PrivateAppTagsResource.get_policy_in_use`.
        """
        return await self._post(
            _TAGS_POLICY_IN_USE_PATH, json={"ids": id_strings(tag_ids, "tag_ids")}
        )
...
class PrivateAppsResource(SyncResource):
    ...
    def get(self, app_id: int) -> PrivateApp:
        ...
        body = self._get(f"{_PATH}/{validate_id(app_id, 'app_id')}")
    ...
    def update(
        ...
        *,
        ...
        """
        body = self._patch(f"{_PATH}/{validate_id(app_id, 'app_id')}", json=extra_fields or {})
        data = body.get("data", body)
        return PrivateApp.model_validate(data)
    ...
    def replace(self, app_id: int, payload: dict[str, Any]) -> PrivateApp:
        ...
        """
        body = self._put(f"{_PATH}/{validate_id(app_id, 'app_id')}", json=payload)
        data = body.get("data", body)
        return PrivateApp.model_validate(data)

    def delete(self, app_id: int) -> None:
        """Delete a private application."""
        self._delete(f"{_PATH}/{validate_id(app_id, 'app_id')}")
    ...
    def get_policy_in_use(self, app_ids: builtins.list[int]) -> dict[str, Any]:
        ...
        (``npa_apps_private.yaml:735-740``), so despite the read-sounding name
        it is not replayed on a 429 or 5xx.
        """
        return self._post(_POLICY_IN_USE_PATH, json={"ids": id_strings(app_ids, "app_ids")})
...
class AsyncPrivateAppsResource(AsyncResource):
    ...
    async def get(self, app_id: int) -> PrivateApp:
        """Get a private app by ID."""
        body = await self._get(f"{_PATH}/{validate_id(app_id, 'app_id')}")
        data = body.get("data", body)
        return PrivateApp.model_validate(data)
    ...
    async def update(
        ...
        *,
        ...
        body = await self._patch(
            f"{_PATH}/{validate_id(app_id, 'app_id')}", json=extra_fields or {}
        )
        data = body.get("data", body)
        return PrivateApp.model_validate(data)
    ...
    async def replace(self, app_id: int, payload: dict[str, Any]) -> PrivateApp:
        """Fully replace a private application (``PUT``)."""
        body = await self._put(f"{_PATH}/{validate_id(app_id, 'app_id')}", json=payload)
        data = body.get("data", body)
        return PrivateApp.model_validate(data)

    async def delete(self, app_id: int) -> None:
        """Delete a private application."""
        await self._delete(f"{_PATH}/{validate_id(app_id, 'app_id')}")
    ...
    async def get_policy_in_use(self, app_ids: builtins.list[int]) -> dict[str, Any]:
        ...
        See :meth:`PrivateAppsResource.get_policy_in_use`.
        """
        return await self._post(_POLICY_IN_USE_PATH, json={"ids": id_strings(app_ids, "app_ids")})
