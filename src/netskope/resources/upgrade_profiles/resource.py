...
from netskope.core.ids import extract_item, id_strings, validate_id
...
class UpgradeProfileResponses(SyncResource):
    ...
    def assign(
        ...
        (``npa_upgrade_profiles.yaml:186-205``) carries the assigned publishers
        under ``data.publishers`` beside ``status`` and ``total``.  The envelope
        is validated whole, so all three reach
        :class:`~netskope.models.infrastructure.UpgradeProfileAssignment`
        instead of the records being descended past.
        """
        payload = _build_assign_payload(profile_id, publisher_ids)
        response = self._transport.request("PUT", _BULK_PATH, json=payload)
        return ApiResponse(
            response, lambda raw: UpgradeProfileAssignment.model_validate(raw.json())
        )
...
class AsyncUpgradeProfileResponses(AsyncResource):
    ...
    async def assign(
        ...
        """Assign publishers to one upgrade profile.

        See :meth:`UpgradeProfileResponses.assign`.
        """
        payload = _build_assign_payload(profile_id, publisher_ids)
...
class UpgradeProfilesResource(SyncResource):
    ...
    def get(self, profile_id: int) -> PublisherUpgradeProfile:
        ...
        """
        body = self._get(f"{_PATH}/{validate_id(profile_id, 'profile_id')}")
    ...
    def update(
        ...
        *,
        ...
        body = self._put(f"{_PATH}/{validate_id(profile_id, 'profile_id')}", json=payload)
        return PublisherUpgradeProfile.model_validate(extract_item(body))
    ...
    def delete(self, profile_id: int) -> None:
        ...
        Args:
            profile_id: The profile identifier (``external_id``).
        """
        self._delete(f"{_PATH}/{validate_id(profile_id, 'profile_id')}")
...
class AsyncUpgradeProfilesResource(AsyncResource):
    ...
    async def get(self, profile_id: int) -> PublisherUpgradeProfile:
        """Get an upgrade profile by ID."""
        body = await self._get(f"{_PATH}/{validate_id(profile_id, 'profile_id')}")
        return PublisherUpgradeProfile.model_validate(extract_item(body))
    ...
    async def update(
        ...
        *,
        ...
        body = await self._put(f"{_PATH}/{validate_id(profile_id, 'profile_id')}", json=payload)
        return PublisherUpgradeProfile.model_validate(extract_item(body))

    async def delete(self, profile_id: int) -> None:
        """Delete an upgrade profile."""
        await self._delete(f"{_PATH}/{validate_id(profile_id, 'profile_id')}")
