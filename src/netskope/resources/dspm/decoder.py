...
The legacy raw methods resolve through this module's route table too and so
reach the same verified paths; they return the unparsed body. This surface only
maps resource names whose semantics are present in dspm_external.yaml at the
pinned gateway contract revision documented in the migration plan, and a name
without a route is rejected rather than turned into a path.
"""
...
from netskope.resources.dspm.paths import _build_list_params, _validate_resource_type
