...
- Wire shapes are checked against a pinned revision of the Netskope API gateway contract, whose definitions are not public. Each resource's test file holds those checks under a `Gateway contract conformance` heading; each assertion's docstring cites the contract file and line it came from, and response fixtures use the contract's own example values. Cite the same way when adding one, and do not copy contract text into the repository
